...
自動化されたプルリクエスト分析とインテリジェントなレビュー洞察：

### Features
- **複雑度分析**: ファイルタイプと変更内容に基づく自動複雑度スコアリング
- **レビュー時間推定**: コードレビューのためのインテリジェントな時間推定
- **変更分類**: ファイルタイプとプロジェクト領域による分析
- **スマート提案**: コンテキストを考慮したレビューフォーカス推奨
...
### 自動統合
PRアナリシスシステムは以下を自動実行します：
- ✅ 全プルリクエストの作成・更新時に分析
- 📊 詳細な分析コメントの投稿
- 🏷️ 複雑度と領域ラベルの追加
- ⏱️ レビュー時間要件の推定

完全な設定とカスタマイゼーションオプションについては [PR Analysis System Documentation](docs/PR_ANALYSIS_SYSTEM.md) を参照してください。
